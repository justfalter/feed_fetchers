...
while total_count < todo_count:
	...
	for x in ilist:
		...
		if x['type'] == 'Host':
			tmp_list = {}
			tmp_list['domain'] = x['summary']
			tmp_list['webLink'] = x['webLink']
			tmp_list['dateAdded'] = x['dateAdded']
			toprint.append(tmp_list)
			del tmp_list
	
print('domain,feedID,killchain,description,webLink,dateAdded')
for x in toprint:
	print("%s,%s,%s,%s,%s,%s" % (x['domain'], feedID, killchain, 'ThreatConnect observed Domain', x['webLink'], x['dateAdded']))
